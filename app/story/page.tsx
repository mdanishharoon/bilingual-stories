...
export default function StoryPage() {
  ...
  const storyTitle =
    typeof story.title === "string"
      ? story.title
      : story.title?.english || "Untitled Story";
  const chineseTitle =
    typeof story.title === "string"
      ? story.chineseTitle
      : story.title?.chinese || "";
  ...
  // Fix the image checking logic
  const hasValidImage =
    currentStoryPage.image &&
    typeof currentStoryPage.image === "string" &&
    currentStoryPage.image.trim() !== "" &&
    !currentStoryPage.image.includes("placeholder.svg");

  const imageSource = hasValidImage
    ? currentStoryPage.image!
    : "/placeholder.svg";
  const isCustomImage = hasValidImage;
          ...
          <div className="text-center mb-4">
            ...
            {story.dedication && (
              <p className="text-sm text-purple-600 italic mt-1">
                {story.dedication}{" "}
                {story.chineseDedication && ` • ${story.chineseDedication}`}
              </p>
            )}
            {isCustomImage && (
              <p className="text-xs text-green-600 font-medium mt-2 flex items-center justify-center">
                <ImageIcon className="w-4 h-4 mr-1" />✨ Featuring custom
